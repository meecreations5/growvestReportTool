...
assert(documentService.includes("viewInvestorDocument") && documentService.includes("getBlob(ref(storage, documentRecord.storagePath))") && documentService.includes("URL.createObjectURL(previewBlob)") && !documentService.includes('window.open("", "_blank")'), "Investor document View creates a temporary protected preview URL without opening a standalone browser tab");
assert(documentPreviewModal.includes('role="dialog"') && documentPreviewModal.includes("<iframe") && documentPreviewModal.includes("<img") && documentPreviewModal.includes("Download"), "secure document popup previews PDF/images in-app and keeps Download available");
assert(investorDocumentsPanel.includes("DocumentPreviewModal") && investorDocumentsPage.includes("DocumentPreviewModal") && investorDocumentsPage.includes("<Eye size={17} /> View"), "Staff Access & Documents and Investor Portal open document View in the shared popup preview");
