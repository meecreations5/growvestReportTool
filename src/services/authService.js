...
export function mapAuthError(error, audience = "staff") {
  ...
  const messages = {
    ...
    "auth/operation-not-allowed": `Firebase rejected the Phone OTP request even though the provider may be enabled. In Firebase Authentication → Settings, allow India (+91) under SMS region policy and add ${typeof window !== "undefined" ? window.location.hostname : "insights.growvest.info"} under Authorized domains. Also confirm the production apiKey, authDomain and projectId all come from the same Firebase web app (${auth.app.options.projectId || "unknown"}).`,
    ...
    "auth/billing-not-enabled": "Firebase Phone Authentication requires billing to be enabled for this project before real SMS messages can be sent.",
