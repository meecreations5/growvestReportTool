      ...
      {mode === "disable" || mode === "enable" ? (
        <Modal onClose={reset} busy={Boolean(busy)}>
          <div className={`rounded-xl border p-4 ${mode === "disable" ? "border-amber-200 bg-amber-50" : "border-emerald-200 bg-emerald-50"}`}>
            ...
            <p className={`mt-1 text-sm leading-6 ${mode === "disable" ? "text-amber-800" : "text-emerald-800"}`}>{mode === "disable" ? "The Investor remains in GrowVest for staff history, but portal login is blocked and lifecycle-paused SIP, Insurance, meeting and scheduled report delivery reminders stop until the Investor is enabled again." : "The Investor returns to active status. SIP reminders paused by Investor disablement resume. Previous portal access is restored only when it had been active before disablement."}</p>
          </div>
          ...
          {preview?.impact ? (
            <div className="mt-5 rounded-xl border border-slate-200 p-4">
              ...
              <div className="mt-2 grid gap-x-6 md:grid-cols-2">
                <div><ImpactRow label="Portfolio holdings" value={preview.impact.portfolioHoldings} /><ImpactRow label="Investment transactions" value={preview.impact.investmentTransactions} /><ImpactRow label="Trading transactions" value={preview.impact.tradingTransactions} /><ImpactRow label="ULIP policies" value={preview.impact.ulipPolicies} /><ImpactRow label="Insurance policies" value={preview.impact.insurancePolicies} /><ImpactRow label="Portfolio snapshots" value={preview.impact.portfolioSnapshots} /></div>
                <div><ImpactRow label="Published / monthly reports" value={preview.impact.monthlyReports} /><ImpactRow label="Documents" value={preview.impact.documents} /><ImpactRow label="Meetings" value={preview.impact.meetings} /><ImpactRow label="Advisor follow-ups" value={preview.impact.advisorFollowUps} /><ImpactRow label="Scheduled report emails" value={preview.impact.scheduledEmailDeliveries} /><ImpactRow label="Portal accounts" value={preview.impact.linkedPortalAccounts} /></div>
              </div>
