...
export default function MonthlyWealthReport({ report, history = [], viewer = "staff" }) {
  ...
  const profileActions = report.profileActions || [];
    ...
    >
      ...
      <section id="report-goals" style={sectionStyle("goals")} className={`scroll-mt-32 ${sectionVisible("goals") ? "" : "hidden"}`}>
        <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
          <div><h2 className="text-2xl font-black text-slate-950">{hasGoals ? "Goals & Bucket List Progress" : "General Wealth Corpus"}</h2><p className="mt-1 text-sm text-slate-400">{hasGoals ? "Goal-by-goal wealth progress" : "Default bucket for investments not linked to a specific Bucket List"}</p></div>
          {viewer === "staff" ? (
        ...
        <div className={`mt-5 grid gap-4 ${goalView === "grid" ? "lg:grid-cols-2 2xl:grid-cols-3" : "grid-cols-1"}`}>
          ...
          {!(viewer === "staff" ? filteredGoals : goals).length ? <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center text-sm text-slate-500">{hasGoals ? "No financial goals match this filter." : <>Investments not linked to a specific Bucket List are mapped to <strong className="text-slate-800">General Wealth (Default)</strong>. Current value: <strong className="text-slate-800">{compactCurrency(summary.generalWealthCorpus || summary.totalCorpus)}</strong>.</>}</div> : null}
        </div>
      </section>
      ...
      <SectionCard id="report-holdings" style={sectionStyle("holdings")} className={`scroll-mt-32 overflow-hidden ${sectionVisible("holdings") ? "" : "hidden"}`}>
        <div className="flex flex-col justify-between gap-4 p-5 sm:p-6 md:flex-row md:items-center"><div><h2 className="text-lg font-black text-slate-950">Investment-wise Holdings</h2><p className="mt-1 text-sm text-slate-400">{funds.length} instruments · {compactCurrency(summary.monthlySip)} monthly SIP · {compactCurrency(summary.totalCorpus)} total value</p></div>{viewer === "staff" ? <button type="button" onClick={() => downloadCsv(`growvest-holdings-${report.reportMonthKey}.csv`, ["Instrument", "Investment Type", "Mode", "Class", "Linked Goal", "Monthly SIP", "Current NAV / Rate", "NAV / Valuation Date", "Current Value", "Weight"], funds.map((item) => [item.instrumentName, item.investmentType || item.productType || "Investment", item.investmentMode || item.type || "—", item.assetClass, item.bucketLabel || item.goalName || "General Wealth (Default)", item.monthlySip, item.currentNav || item.currentRate || "", item.navDate || item.valuationDate || "", item.currentValue, summary.totalCorpus ? ((Number(item.currentValue || 0) / Number(summary.totalCorpus)) * 100).toFixed(1) : 0]))} className="inline-flex items-center gap-2 rounded-lg border border-slate-200 px-4 py-2 text-sm font-bold text-slate-600"><Download size={15} /> Export</button> : null}</div>
        <div className="grid gap-3 px-4 pb-5 md:hidden">{funds.map((item) => { const weight = summary.totalCorpus ? (Number(item.currentValue || 0) / Number(summary.totalCorpus)) * 100 : 0; const navOrRate = Number(item.currentNav || item.currentRate || 0); const valuationDate = item.navDate || item.valuationDate || ""; return <article key={item.id} className="rounded-2xl border border-slate-200 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-black text-slate-950">{item.instrumentName}</p><p className="mt-1 text-xs text-slate-500">{item.investmentType || "Investment"} · {item.investmentMode || item.type || "—"}</p>{item.productType === "ulip" && item.policyNumber ? <p className="mt-1 text-xs text-violet-600">Policy {item.policyNumber}{item.planName ? ` · ${item.planName}` : ""}</p> : null}<p className="mt-1 text-xs text-slate-400">{item.assetClass} · {item.bucketLabel || item.goalName || "General Wealth (Default)"}</p></div><span className="rounded-full bg-blue-50 px-2.5 py-1 text-[10px] font-bold text-blue-700">{weight.toFixed(1)}%</span></div><div className="mt-4 grid grid-cols-2 gap-3 text-sm"><div><span className="text-xs text-slate-400">Current value</span><strong className="mt-1 block text-slate-900">{compactCurrency(item.currentValue)}</strong></div><div><span className="text-xs text-slate-400">Monthly SIP</span><strong className="mt-1 block text-slate-900">{Number(item.monthlySip || 0) ? compactCurrency(item.monthlySip) : "—"}</strong></div><div><span className="text-xs text-slate-400">NAV / Rate</span><strong className="mt-1 block text-slate-900">{navOrRate ? compactCurrency(navOrRate) : "—"}</strong></div><div><span className="text-xs text-slate-400">Valuation date</span><strong className="mt-1 block text-slate-900">{valuationDate || "—"}</strong></div></div><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-blue-600" style={{ width: `${Math.min(100, weight)}%` }} /></div></article>; })}{!funds.length ? <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center text-sm text-slate-500">No investment holdings have been added.</div> : null}</div>
        <div className="hidden overflow-x-auto md:block"><table className="min-w-[1180px] w-full text-left text-sm"><thead className="border-y border-slate-200 bg-slate-50 text-xs text-slate-500"><tr>{["Instrument", "Investment Type", "Mode", "Class", "Linked Goal", "NAV / Rate", "Valuation Date", "Monthly SIP", "Current Value", "Weight"].map((item) => <th key={item} className="px-5 py-4 font-bold">{item}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{funds.map((item) => { const weight = summary.totalCorpus ? (Number(item.currentValue || 0) / Number(summary.totalCorpus)) * 100 : 0; const navOrRate = Number(item.currentNav || item.currentRate || 0); return <tr key={item.id}><td className="px-5 py-4 font-black text-slate-950"><div>{item.instrumentName}</div>{item.productType === "ulip" && item.policyNumber ? <div className="mt-1 text-[10px] font-semibold text-violet-600">Policy {item.policyNumber}{item.fundCode ? ` · ${item.fundCode}` : ""}</div> : null}</td><td className="px-5 py-4 text-slate-600">{item.investmentType || "Investment"}</td><td className="px-5 py-4 text-slate-500">{item.investmentMode || item.type || "—"}</td><td className="px-5 py-4"><span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-bold text-blue-700">{item.assetClass}</span></td><td className="px-5 py-4 text-slate-500">{item.bucketLabel || item.goalName || "General Wealth (Default)"}</td><td className="px-5 py-4 font-semibold">{navOrRate ? compactCurrency(navOrRate) : "—"}</td><td className="px-5 py-4 text-slate-500">{item.navDate || item.valuationDate || "—"}</td><td className="px-5 py-4 font-bold">{Number(item.monthlySip || 0) ? compactCurrency(item.monthlySip) : "—"}</td><td className="px-5 py-4 font-black">{compactCurrency(item.currentValue)}</td><td className="px-5 py-4"><div className="flex items-center gap-3"><div className="h-1.5 w-20 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-blue-600" style={{ width: `${Math.min(100, weight)}%` }} /></div><span className="text-slate-500">{weight.toFixed(1)}%</span></div></td></tr>; })}</tbody></table></div>
      </SectionCard>
      ...
      {tradingSummary && Number(tradingSummary.totalTrades || 0) > 0 ? <SectionCard id="report-trading" style={sectionStyle("holdings", 1)} className="scroll-mt-32 p-5 sm:p-6">
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
          {[
            ...
            ["Gross P&L", compactCurrency(tradingSummary.grossPnl || 0), "Before broker/statutory charges"],
            ["Total Charges", compactCurrency(tradingSummary.totalCharges || 0), "Brokerage and recorded charges"],
            ["Net Realised P&L", compactCurrency(tradingSummary.netPnl || 0), "Not automatically added to goal corpus"]
          ].map(([label, value, helper]) => <div key={label} className="rounded-xl border border-slate-200 bg-slate-50 p-4"><p className="text-[10px] font-bold uppercase tracking-wide text-slate-400">{label}</p><p className="mt-2 text-xl font-black text-slate-950">{value}</p><p className="mt-1 text-xs text-slate-500">{helper}</p></div>)}
        </div>
      </SectionCard> : null}
      ...
      {protectionSnapshot && (protectionPolicies.length || Number(protectionSummary.activePolicyCount || 0) > 0) ? <SectionCard id="report-protection" style={sectionStyle("commentary", -2)} className="scroll-mt-32 p-5 sm:p-6">
        ...
        {protectionPolicies.length ? <>
          <div className="mt-5 grid gap-3 md:hidden">{protectionPolicies.slice(0,12).map((item) => <article key={item.id || item.policyNumber} className="rounded-2xl border border-slate-200 bg-white p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-[9px] font-black uppercase tracking-[.1em] text-blue-700">{item.insuranceType || "Protection"}</p><p className="mt-1 line-clamp-2 font-black text-slate-950">{item.productName || item.planName || "Insurance Policy"}</p><p className="mt-1 line-clamp-1 text-[10px] text-slate-400">{item.insurer || "Insurer"}{item.policyNumber ? ` · ${item.policyNumber}` : ""}</p></div><span className="shrink-0 rounded-full bg-emerald-50 px-2.5 py-1 text-[9px] font-black text-emerald-700">{item.policyStatus || "Active"}</span></div><div className="mt-4 grid grid-cols-2 gap-2 rounded-xl bg-slate-50 p-3"><div><p className="text-[8px] font-bold uppercase text-slate-400">Cover</p><p className="mt-1 text-xs font-black text-slate-900">{compactCurrency(item.coverAmount || 0)}</p></div><div><p className="text-[8px] font-bold uppercase text-slate-400">Premium</p><p className="mt-1 text-xs font-black text-slate-900">{compactCurrency(item.premiumAmount || 0)}</p><p className="mt-0.5 text-[8px] text-slate-400">{item.premiumFrequency || "—"}</p></div></div>{item.nextDueDate ? <p className="mt-3 text-[10px] font-bold text-amber-700">{item.nextDueType || "Next due"} · {formatDate(item.nextDueDate)}</p> : null}</article>)}</div>
          <div className="mt-5 hidden overflow-x-auto rounded-xl border border-slate-200 md:block"><table className="min-w-[760px] w-full text-left text-sm"><thead className="bg-slate-50 text-xs text-slate-500"><tr>{["Type", "Policy", "Cover", "Premium", "Next Due", "Status"].map((label) => <th key={label} className="px-4 py-3 font-bold">{label}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{protectionPolicies.slice(0,12).map((item) => <tr key={item.id || item.policyNumber}><td className="px-4 py-3 font-semibold text-slate-700">{item.insuranceType}</td><td className="px-4 py-3"><p className="font-bold text-slate-950">{item.productName}</p><p className="text-xs text-slate-400">{item.insurer} · {item.policyNumber}</p></td><td className="px-4 py-3 font-bold">{compactCurrency(item.coverAmount || 0)}</td><td className="px-4 py-3">{compactCurrency(item.premiumAmount || 0)}{item.premiumFrequency ? ` · ${item.premiumFrequency}` : ""}</td><td className="px-4 py-3">{item.nextDueDate ? `${item.nextDueType || "Due"} · ${formatDate(item.nextDueDate)}` : "—"}</td><td className="px-4 py-3 font-semibold">{item.policyStatus || "Active"}</td></tr>)}</tbody></table></div>
        </> : null}
      </SectionCard> : null}
      ...
      {(Number(financialPlan.monthlySurplus || 0) > 0 || surplusAllocations.length || loans.length) ? <SectionCard id="report-financial-plan" style={sectionStyle("commentary", -1)} className="scroll-mt-32 p-5 sm:p-6">
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-2">
          <div className="rounded-xl border border-slate-200 p-4"><div className="flex items-center justify-between gap-3"><p className="font-black text-slate-950">Monthly Surplus</p><strong className="text-xl text-blue-700">{compactCurrency(financialPlan.monthlySurplus || 0)}</strong></div><p className="mt-1 text-xs text-slate-500">{financialPlan.surplusMode === "percentage" ? `${Number(financialPlan.surplusPercentage || 0)}% of monthly income` : "Fixed monthly amount"}</p><div className="mt-4 grid gap-2">{surplusAllocations.length ? surplusAllocations.map((item) => <div key={item.id} className="flex items-center justify-between gap-4 rounded-lg bg-slate-50 px-3 py-2 text-sm"><span className="text-slate-600">{item.category}</span><strong className="text-slate-950">{compactCurrency(item.calculatedAmount || 0)}{item.mode === "percentage" ? ` · ${Number(item.percentage || 0)}%` : ""}</strong></div>) : <p className="rounded-lg bg-slate-50 p-3 text-sm text-slate-500">No surplus allocation plan recorded.</p>}</div></div>
          <div className="rounded-xl border border-slate-200 p-4"><div className="flex items-center justify-between gap-3"><p className="font-black text-slate-950">Active Loans / Liabilities</p><strong className="text-xl text-red-700">{compactCurrency(loans.reduce((sum, item) => sum + Number(item.outstandingAmount || 0), 0))}</strong></div><div className="mt-4 grid gap-2">{loans.length ? loans.map((item) => <div key={item.id} className="rounded-lg bg-slate-50 px-3 py-2"><div className="flex items-center justify-between gap-3 text-sm"><span className="font-semibold text-slate-700">{item.type}{item.lender ? ` · ${item.lender}` : ""}</span><strong className="text-slate-950">{compactCurrency(item.outstandingAmount || 0)}</strong></div><p className="mt-1 text-xs text-slate-500">EMI {compactCurrency(item.emiAmount || 0)}{Number(item.interestRate || 0) ? ` · ${Number(item.interestRate).toFixed(2)}%` : ""}{Number(item.extraRepayment || 0) ? ` · Extra ${compactCurrency(item.extraRepayment)}` : ""}{item.targetClosureDate ? ` · Target ${formatDate(item.targetClosureDate)}` : ""}</p></div>) : <p className="rounded-lg bg-slate-50 p-3 text-sm text-slate-500">No active liabilities recorded.</p>}</div></div>
        </div>
      </SectionCard> : null}
