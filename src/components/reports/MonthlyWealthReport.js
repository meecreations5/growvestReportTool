    ...
    >
      ...
      <SectionCard id="report-performance" style={sectionStyle("executiveSummary")} className={`scroll-mt-32 p-5 sm:p-6 ${sectionVisible("executiveSummary") ? "" : "hidden"}`}>
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-start">
          <div>
            ...
            </> : <div className="mt-5 rounded-xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm text-blue-900"><span className="font-black">General Wealth Corpus:</span> {compactCurrency(summary.generalWealthCorpus || summary.totalCorpus)} · No specific financial goal is required.</div>}
          </div>
      ...
      <div style={sectionStyle("executiveSummary", 1)} className={`grid gap-4 sm:grid-cols-2 xl:grid-cols-4 ${sectionVisible("executiveSummary") ? "" : "hidden"}`}>
        {[
          ...
          [hasGoals ? "Active Financial Goals" : "General Wealth Corpus", hasGoals ? activeGoals : compactCurrency(summary.generalWealthCorpus || summary.totalCorpus), hasGoals ? `${onTrackGoals} on track · ${attentionGoals} need attention` : "No specific goal assigned", Target, "text-amber-700 bg-amber-50"]
      ...
      <section id="report-goals" style={sectionStyle("goals")} className={`scroll-mt-32 ${sectionVisible("goals") ? "" : "hidden"}`}>
        <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
          <div><h2 className="text-2xl font-black text-slate-950">{hasGoals ? "Goals & Bucket List Progress" : "General Wealth Corpus"}</h2><p className="mt-1 text-sm text-slate-400">{hasGoals ? "Goal-by-goal wealth progress" : "Portfolio growth without a specific goal assignment"}</p></div>
          {viewer === "staff" ? (
        ...
        <div className={`mt-5 grid gap-4 ${goalView === "grid" ? "lg:grid-cols-2 2xl:grid-cols-3" : "grid-cols-1"}`}>
          ...
          {!(viewer === "staff" ? filteredGoals : goals).length ? <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center text-sm text-slate-500">{hasGoals ? "No financial goals match this filter." : <>No specific goal is defined. <strong className="text-slate-800">{compactCurrency(summary.generalWealthCorpus || summary.totalCorpus)}</strong> is tracked as General Wealth Corpus and can be allocated to goals later.</>}</div> : null}
        </div>
      </section>
