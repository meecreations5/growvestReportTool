...
import { Check, Eye, FileText, LayoutTemplate, Sparkles } from "lucide-react";
...
export default function ReportTemplateSelectionStep({ templates = [], selectedTemplateId, onSelect, disabled = false }) {
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 2xl:grid-cols-3">
      {available.map((template) => {
        ...
        return (
          <article key={template.id} className={`overflow-hidden rounded-xl border bg-white transition ${selected ? "border-blue-500 ring-4 ring-blue-50" : "border-slate-200 hover:border-blue-200"}`}>
            ...
            <div className="p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                {selected ? <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-blue-700 text-white"><Check size={16} /></span> : null}
              </div>
              ...
              <div className="mt-4 grid grid-cols-[1fr_auto] gap-2">
                <button type="button" disabled={disabled} onClick={() => onSelect(template)} className={`inline-flex min-h-11 items-center justify-center gap-2 rounded-lg px-4 text-sm font-semibold transition disabled:opacity-60 ${selected ? "bg-blue-50 text-blue-700 ring-1 ring-inset ring-blue-200" : "bg-[#1F4ED8] text-white hover:bg-[#173EB4]"}`}>
                  {selected ? <Check size={16} /> : <LayoutTemplate size={16} />}
