...
export default function MonthlyReportPrintDocument({ report, history = [] }) {
  ...
  const renderers = {
    ...
    goals() {
      chunks(goals, 6).forEach((pageGoals, index) => addPage(`goals-${index}`, index === 0 ? (hasGoals ? "GOALS & BUCKET LIST PROGRESS" : "GENERAL WEALTH CORPUS") : "GOAL PROGRESS — CONTINUED", <><p className="report-intro">{hasGoals ? "Every financial goal in your plan, its target and how close you are today." : "This portfolio is currently tracked as General Wealth without a specific financial goal."}</p><div className="mt-5 grid grid-cols-2 gap-4">{pageGoals.length ? pageGoals.map((goal) => <GoalPrintCard key={goal.goalId || goal.id || goal.name} goal={goal} />) : <div className="col-span-2"><EmptyPrintState>General Wealth Corpus: <strong>{compactCurrency(summary.generalWealthCorpus || summary.totalCorpus)}</strong>. Investments can be allocated to goals later.</EmptyPrintState></div>}</div></>));
    },
    allocation() {
