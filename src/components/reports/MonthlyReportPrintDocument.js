...
export default function MonthlyReportPrintDocument({ report, history = [] }) {
  ...
  const renderers = {
    ...
    commentary() {
      ...
      const protectionPolicies = protection?.policies || [];
      const protectionSummary = protection?.summary || {};
      if (protection && (protectionPolicies.length || Number(protectionSummary.activePolicyCount || 0))) {
        addPage("protection", "INSURANCE & PROTECTION", <><p className="report-intro">Protection coverage is shown separately and is not included in the investment portfolio corpus.</p><div className="grid grid-cols-4 gap-3"><div className="report-stat"><span>ACTIVE POLICIES</span><strong>{Number(protectionSummary.activePolicyCount || 0)}</strong></div><div className="report-stat"><span>LIFE COVER</span><strong>{compactCurrency(protectionSummary.lifeCover || 0)}</strong></div><div className="report-stat"><span>HEALTH COVER</span><strong>{compactCurrency(protectionSummary.healthCover || 0)}</strong></div><div className="report-stat"><span>DUE ≤30 DAYS</span><strong>{Number(protectionSummary.policiesExpiringWithin30Days || 0) + Number(protectionSummary.premiumsDueWithin30Days || 0)}</strong></div></div>{protectionPolicies.length ? <ReportTable headers={["Type", "Policy", "Cover", "Premium", "Next Due", "Status"]}>{protectionPolicies.slice(0,10).map((item) => <tr key={item.id || item.policyNumber}><td>{item.insuranceType}</td><td><strong>{item.productName}</strong><br/><span>{item.insurer} · {item.policyNumber}</span></td><td>{compactCurrency(item.coverAmount || 0)}</td><td>{compactCurrency(item.premiumAmount || 0)}</td><td>{item.nextDueDate ? `${item.nextDueType || "Due"} · ${formatDate(item.nextDueDate)}` : "—"}</td><td>{item.policyStatus || "Active"}</td></tr>)}</ReportTable> : <EmptyPrintState>No active insurance policy details were included in this report.</EmptyPrintState>}</>);
      }
