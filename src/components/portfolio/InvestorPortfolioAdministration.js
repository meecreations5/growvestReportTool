...
export default function InvestorPortfolioAdministration({ investorId }) {
  ...
  return <div className="grid gap-6">
    ...
    <Card className="border-red-200 bg-red-50/30 p-5"><div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center"><div><p className="text-[10px] font-bold uppercase tracking-[0.12em] text-red-700">Danger zone</p><h2 className="mt-1 font-heading text-xl font-bold text-slate-950">Delete entire investor portfolio data</h2><p className="mt-1 max-w-3xl text-sm leading-6 text-slate-600">Removes all current holdings plus all Trading / Intraday records for this investor. Profile, KYC, documents, Goals/Bucket Lists, meetings, Advisor Follow-ups, Service Requests and published Monthly Reports remain preserved.</p></div><Button type="button" variant="danger" onClick={openFullDelete} disabled={fullBusy || (!positions.length && !trades.length)}>{fullBusy ? <Loader2 size={16} className="animate-spin" /> : <Trash2 size={16} />} Delete Entire Portfolio</Button></div></Card>
    ...
    {fullDialog ? <div className="fixed inset-0 z-[180] flex items-end justify-center bg-slate-950/60 p-0 sm:items-center sm:p-4" role="dialog" aria-modal="true"><button type="button" className="absolute inset-0" onClick={() => fullBusy ? null : setFullDialog(false)} /><section className="relative z-10 w-full max-w-2xl rounded-t-[28px] bg-white p-5 shadow-2xl sm:rounded-2xl sm:p-6"><div className="flex items-start gap-3"><ShieldAlert className="mt-0.5 shrink-0 text-red-700" /><div><p className="text-[10px] font-bold uppercase tracking-wide text-red-700">Entire portfolio cleanup</p><h2 className="mt-1 font-heading text-2xl font-bold text-slate-950">Delete all current portfolio data</h2><p className="mt-2 text-sm leading-6 text-slate-500">{fullPreview?.holdings || 0} holding(s), {fullPreview?.transactions || 0} related imported transaction(s), {fullPreview?.trades || 0} trading record(s) and {fullPreview?.summaries || 0} trading summary record(s) will be removed. Current holding value affected: {formatCurrency(fullPreview?.currentValue)}.</p></div></div><div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs leading-5 text-amber-900">Published Monthly Reports stay frozen. The current portfolio and Goal/Bucket corpus will be recalculated, and the cleanup remains available in audit history.</div><div className="mt-5 grid gap-4"><label className="grid gap-2"><span className="text-xs font-bold text-slate-700">Reason</span><textarea rows={3} className={inputClassName} value={fullReason} onChange={(e) => setFullReason(e.target.value)} /></label><label className="grid gap-2"><span className="text-xs font-bold text-slate-700">Type DELETE</span><input className={inputClassName} value={fullConfirm} onChange={(e) => setFullConfirm(e.target.value)} /></label></div><div className="mt-6 flex justify-end gap-2"><Button type="button" variant="secondary" onClick={() => setFullDialog(false)} disabled={fullBusy}>Cancel</Button><Button type="button" variant="danger" onClick={confirmFullDelete} disabled={fullBusy || fullReason.trim().length < 5 || fullConfirm.trim().toUpperCase() !== "DELETE"}>{fullBusy ? <Loader2 size={16} className="animate-spin" /> : <Trash2 size={16} />} Delete Entire Portfolio</Button></div></section></div> : null}
