...
export default function ManualPortfolioExcelPanel({ investorId, onImported }) {
  ...
  async function downloadTemplate() {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = "GrowVest_Manual_Portfolio_Template_v0.33.0.xlsx";
      document.body.appendChild(anchor);
      anchor.click();
      anchor.remove();
      URL.revokeObjectURL(url);
    } catch (nextError) { setError(nextError.message || "Unable to download the Manual Portfolio template."); }
    finally { setBusy(""); }
  }
  ...
  return <Card className="p-5 sm:p-6">
    <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-blue-700">Manual account management</p><h2 className="mt-1 font-heading text-2xl font-bold text-slate-950">Upload Manual Portfolio Excel</h2><p className="mt-1 max-w-3xl text-sm leading-6 text-slate-500">Use this when GrowVest manages this investor's account manually. The selected investor is already known, so the Excel file only needs the holdings.</p></div><Button type="button" variant="secondary" onClick={downloadTemplate} disabled={Boolean(busy)}>{busy === "template" ? <Loader2 size={16} className="animate-spin" /> : <FileSpreadsheet size={16} />} Download Template</Button></div>
